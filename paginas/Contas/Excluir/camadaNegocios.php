...
$tb = 'Conta';
$tb2 = 'Recebimento';
$send = filter_input(INPUT_POST, 'excluirContaSubmit', FILTER_SANITIZE_STRING);
if ($send) {
    ...
    try {
		...
		foreach($select_msg_cont->fetchAll() as $linha_array){
			if($linha_array['quantidade'] < 1){
				$_SESSION['excluirContaFalha'] = "Item inexistente, não pode ser deletado!";
				header("Location: ./excluirContas.php");
			}
		}
		if(!(isset($_SESSION['excluirContaFalha']))){
        $result_msg_cont = "DELETE FROM $db.$tb2 WHERE Conta_codigoConta=:codigo";
        $delete_msg_cont = $conx->prepare($result_msg_cont);
        $delete_msg_cont->bindParam(':codigo', $codigo);
        $delete_msg_cont->execute();

        $result_msg_cont = "DELETE FROM $db.$tb WHERE codigoConta=:codigo";
        $delete_msg_cont = $conx->prepare($result_msg_cont);
        $delete_msg_cont->bindParam(':codigo', $codigo);
        $delete_msg_cont->execute();
