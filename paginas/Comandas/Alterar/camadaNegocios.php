...
if ($send) {
	...
	$codigoComanda = filter_input(INPUT_POST, 'codigoComanda', FILTER_SANITIZE_NUMBER_INT);
	...
	try {
		//Variável para armazenar erro nos primeiros testes
		$erro = "";

		//Verifica se comanda está aberta
		$result_msg_cont = "SELECT fechadaComanda from $db.$tb WHERE codigoComanda=:codigoComanda";
		$select_msg_cont = $conx->prepare($result_msg_cont);
		$select_msg_cont->execute([':codigoComanda' => $codigoComanda]);
		$select_msg_cont = $select_msg_cont->fetchAll();
		if (!empty($select_msg_cont)) {
			foreach ($select_msg_cont as $linha_array) {
				if ($linha_array['fechadaComanda'] == 1) {
					$erro = "NÃO É POSSÍVEL ALTERAR COMANDAS FECHADAS. COMANDA NÃO ALTERADA!";
				}
			}
		} else {
			$erro = "A COMANDA ALTERADA NÃO ESTAVA CADASTRADA ANTERIORMENTE. COMANDA NÃO ALTERADA!";
		}
		...
		if ($erro == "") {
			...
			if ((($totalRecebimento >= $valorTotal and $status == 1) or ($status == 0)) and $valorTotal > 0 and (($totalRecebimento > 0 and $status == 1) or ($status == 0)) and $valorTotal <= 999999999.99 and $totalRecebimento <= 999999999.99 and (($cadastrada == 0 and $status == 0) or ($cadastrada == 1 and $status == 1))) {
				//Atualiza comanda no banco de dados
				$result_msg_cont = "UPDATE $db.$tb SET valorTotalComanda=:valorTotalComanda, fechadaComanda=:status, trocoComanda=:trocoComanda WHERE codigoComanda=:codigoComanda";
				...
				$update_msg_cont->execute([':codigoComanda' => $codigoComanda, ':valorTotalComanda' => $valorTotal, ':status' => $status, ':trocoComanda' => $troco]);

				//Encontra o código do atendimento corresponde a essa comanda
				$result_msg_cont = "SELECT codigoAtendimento FROM $db.$tb6 WHERE Comanda_codigoComanda =:codigoComanda";
				$select_msg_cont = $conx->prepare($result_msg_cont);
				$select_msg_cont->execute([':codigoComanda' => $codigoComanda]);
				$select_msg_cont = $select_msg_cont->fetchAll();
				$codigoAtendimento = 0;
				foreach ($select_msg_cont as $linha_array) {
					$codigoAtendimento = $linha_array['codigoAtendimento'];
				}

				//Atualiza o atendimento no banco de dados
				$result_msg_cont = "UPDATE $db.$tb6 SET dataHoraAtendimento=now(),Funcionario_codigoFuncionario=:codigoFuncionario, Comanda_codigoComanda=:codigoComanda, status=:status where codigoAtendimento=:codigoAtendimento";
				$insert_msg_cont = $conx->prepare($result_msg_cont);
				$codigoFuncionario = $_SESSION['loginCodigo'];
				$insert_msg_cont->execute([':codigoFuncionario' => $codigoFuncionario, ':codigoComanda' => $codigoComanda, ':codigoAtendimento' => $codigoAtendimento, ':status' => $status]);

				//Remove os itens da comanda que estão no banco de dados
				$result_msg_cont = "DELETE FROM $db.$tb5 WHERE Comanda_codigoComanda=:codigoComanda";
				$delete_msg_cont = $conx->prepare($result_msg_cont);
				$delete_msg_cont->bindParam(':codigoComanda', $codigoComanda);
				...
				foreach ($itensArray as $itemArray) {
					...
					$insert_msg_cont->bindParam(':codigoComanda', $codigoComanda);
				...
				if ($cadastrada == 1 and $status == 1) {
					...
					if ($quantiaDinheiro > 0) {
						...
						$insert_msg_cont = $conx->prepare($result_msg_cont);
						$metodo = "Dinheiro";
						$insert_msg_cont->execute(['metodoPagamentoRecebimento' => $metodo, 'Conta_codigoConta' => $codigoConta, 'Comanda_codigoComanda' => $codigoComanda, 'valorRecebimento' => $quantiaDinheiro]);
					}
					...
					if ($quantiaCartao > 0) {
						...
						$insert_msg_cont = $conx->prepare($result_msg_cont);
						$metodo = "Cartão";
						$insert_msg_cont->execute(['metodoPagamentoRecebimento' => $metodo, 'codigoConta' => $codigoConta, 'codigoComanda' => $codigoComanda, 'valorRecebimento' => $quantiaCartao]);
					}
					...
					if ($quantiaPix > 0) {
						...
						$insert_msg_cont = $conx->prepare($result_msg_cont);
						$metodo = "Pix";
						$insert_msg_cont->execute(['metodoPagamentoRecebimento' => $metodo, 'codigoConta' => $codigoConta, 'codigoComanda' => $codigoComanda, 'valorRecebimento' => $quantiaPix]);
					}
				}
